double jaro_distance(string &a, string &b)
{
	if(a == b){
		return 1.0;	
	} 
	...
	// Count number of occurances where two characters match but there is a third matched character in between the indices 
    for(int i = 0; i < lena; i++){
    	if(hash_a[i])
    	{ 
    ...
    return (((double)match)/((double)lena) + ((double)match)/((double)lenb) + ((double)match - t)/((double)match))/3.0;
}

double jaro_winkler(string &a, string &b)
{
	double jaro_dis = jaro_distance(a,b);
